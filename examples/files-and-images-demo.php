...
try {
    ...
    if (file_exists($structuredImagePath) && file_exists($chaoticImagePath)) {
        ...
        if (!file_exists($structuredImagePath)) echo "  - Not found: {$structuredImagePath}\n";
        if (!file_exists($chaoticImagePath)) echo "  - Not found: {$chaoticImagePath}\n";
    }
} catch (\Exception $e) {
    echo "Error: {$e->getMessage()}\n";
}
...
try {
    ...
    if (file_exists($simplePdfPath) && file_exists($structuredImagePath)) {
        ...
        if (!file_exists($simplePdfPath)) echo "  - Not found: {$simplePdfPath}\n";
        if (!file_exists($structuredImagePath)) echo "  - Not found: {$structuredImagePath}\n";
    }
} catch (\Exception $e) {
    echo "Error: {$e->getMessage()}\n";
...
try {
    ...
    if (!empty($files)) {
        ...
        if (!file_exists($simplePdfPath)) echo "  - Not found: {$simplePdfPath}\n";
        if (!file_exists($hardPdfPath)) echo "  - Not found: {$hardPdfPath}\n";
    }
} catch (\Exception $e) {
    echo "Error: {$e->getMessage()}\n";
}
